...
> **连接容错**：验证时客户端会依次尝试以下地址（各带 http/https），只要有一个服务器能连通就停止：
> `play.simpfun.cn:14639`、`yh-team.org`、`3c3u.org`。仅在“连不上”时才切换下一个，若某地址返回了明确的业务结果（如密码错误）则立即采用该结果。
>
> **调试模式**：验证窗口勾选 “Debug 调试模式” 后，会把每次访问的 URL、HTTP 状态码与原始响应内容用信息框显示，方便排查“网络错误”。开关会写入 `nyx-auth.properties` 的 `debug=true`。
